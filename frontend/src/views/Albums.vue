  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10">
    ...
    <div class="mb-10">
      <div class="grid grid-cols-2 sm:flex sm:justify-center gap-2.5 max-w-xl mx-auto p-1.5 rounded-2xl liquid-card">
        <button v-for="(tab, idx) in albumTabs" :key="tab.id" @click="activeTab = tab.id"
    ...
    <div v-else-if="currentAlbumData" class="space-y-10">
      ...
      <div
        class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200/60 dark:border-gray-800/60 pb-5">
        <div>
        ...
        <div class="inline-flex p-1 rounded-xl liquid-card self-start sm:self-auto">
          <button @click="viewMode = 'models'" class="px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all"
          ...
          <button @click="viewMode = 'gallery'" class="px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all"
            ...
            <AppIcon name="image" :size="16" /> Галерея
          </button>
        </div>
      </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
            ...
            <div class="lg:col-span-5 space-y-3">
              ...
              <div
                ...
                <img :src="'/uploads/thumbs/' + (model.cover_image || model.images[0])"
                  @error="(e) => (e.target as HTMLImageElement).src = '/uploads/' + (model.cover_image || model.images[0])"
                  :alt="model.name"
                  class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                  loading="lazy" />
                <div
                  class="absolute inset-0 bg-black/0 group-hover:bg-black/25 transition-colors flex items-center justify-center">
                  <span
                    class="opacity-0 group-hover:opacity-100 px-3.5 py-1.5 bg-black/70 backdrop-blur-md text-white text-xs font-bold uppercase tracking-wider rounded-lg transition-opacity">
                ...
                <!-- Badge on Top Left -->
                <div
                  class="absolute top-3 left-3 bg-slate-900/80 backdrop-blur-md text-white text-[11px] font-extrabold uppercase tracking-wider px-3 py-1 rounded-lg">
              ...
              <div v-if="model.images && model.images.length > 1" class="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                <button v-for="(img, imgIdx) in model.images" :key="img" @click="openModelLightbox(model, imgIdx)"
                  ...
                  <img :src="'/uploads/thumbs/' + img"
                    @error="(e) => (e.target as HTMLImageElement).src = '/uploads/' + img" :alt="model.name"
            ...
            <div class="lg:col-span-7 flex flex-col justify-between h-full space-y-5">
              <div>
                ...
                <div class="flex flex-wrap items-center gap-2 mb-2.5">
                  <span
                    class="px-2.5 py-1 rounded-md bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 font-bold text-[11px] uppercase tracking-wide">
                    {{ model.format || 'Формат' }}
                  </span>
                  <span
                    class="px-2.5 py-1 rounded-md bg-slate-200/70 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-medium text-[11px]">
                ...
                <!-- Description Text -->
                <p
                  class="text-xs sm:text-sm text-slate-600 dark:text-slate-300 mt-3 leading-relaxed whitespace-pre-line">
                  {{ getCleanShortDesc(model.description) }}
                </p>

                <!-- Spread Options Pricing Grid (if available) -->
                <div v-if="model.spread_options && model.spread_options.length"
                  class="mt-4 pt-4 border-t border-slate-200/60 dark:border-slate-800/80">
                  ...
                  <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    <div v-for="opt in model.spread_options" :key="opt.spreads"
                ...
                <div
                  class="mt-4 flex flex-wrap gap-x-4 gap-y-1.5 text-[11px] text-slate-500 dark:text-slate-400 font-medium">
              ...
              <div
                class="pt-4 border-t border-slate-200/60 dark:border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  ...
                  <span class="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white font-mono">
                    {{ model.spread_options && model.spread_options.length ? 'от ' +
                      formatRub(model.spread_options[0].price) : formatRub(model.price) }}
                  </span>
                </div>

                <router-link :to="{ path: '/contacts', query: { subject: model.name } }"
      ...
      <div v-else-if="viewMode === 'gallery'" class="space-y-6">
        <UniformPhotoGrid :images="currentAlbumData.images" @select="openGalleryLightbox" />
      </div>

    </div>

    <!-- Lightbox Modal -->
    <LightboxModal v-if="lightboxImages.length" :images="lightboxImages" v-model="selectedImageIdx" />
  </div>
</template>
